...
namespace ApiCore3AndTests.Infra.Configurations.Filters.Cache
{
    ...
    public sealed class CacheFilterAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            _body = JsonConvert.SerializeObject(context.ActionArguments.Where(x => !x.Key.Contains("correlationid", StringComparison.InvariantCultureIgnoreCase)));
            var separotor = (string.IsNullOrWhiteSpace(Folder) ? string.Empty : ":");
            _hashKey = $"{Folder}{separotor}{string.Join("-", new[] { _path.Replace("/", "-"), GetStringSha256Hash(_body) })}";
